...
# -------------------------------------------
# remove predictors with too large correlations:
# corr_bre_can_dgn <- cor(bre_can_dgn)
#
high_corr_pred <- findCorrelation(corr_bre_can_dgn, cutoff = .9)
length(high_corr_pred) 
str(high_corr_pred)

colnames(soybean[high_corr_pred])
...
for (col in 1:ncol(x_trans_pca_bre_can_dgn)) { 
  ...
  hist(x_trans_pca_bre_can_dgn[,col],  
       main=paste(paste(paste(colnames(x_trans_pca_bre_can_dgn[col]),":"),col_alias[col])),
       xlab=paste("x_trans_pca_bre_can_dgn$",colnames(x_trans_pca_bre_can_dgn[col])),
       col="#e177bc", border=0)
  mtext(paste('skewness: ', round(skewness(x_trans_pca_bre_can_dgn[,col]), 8)), cex=0.6)
}
